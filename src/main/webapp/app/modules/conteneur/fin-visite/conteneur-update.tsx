...
export const ConteneurUpdate = (props: RouteComponentProps<{ id: string }>) => {
  ...
  const conteneurEntity = useAppSelector(state => state.conteneurFinVisite.entity);
  const loading = useAppSelector(state => state.conteneurFinVisite.loading);
  const updating = useAppSelector(state => state.conteneurFinVisite.updating);
  const updateSuccess = useAppSelector(state => state.conteneurFinVisite.updateSuccess);

  const handleClose = () => {
    props.history.push('/conteneur/finvisite');
  };
  ...
  return (
    <div>
      <Row className="justify-content-center">
        <Col md="8">
          <h2 id="gestionConteneurApp.conteneur.home.finVisiteLabel" data-cy="FinVisiteHeading">
            <Translate contentKey="gestionConteneurApp.conteneur.home.finVisiteLabel">Visiter</Translate>
          </h2>
        </Col>
      </Row>
      <Row className="justify-content-center">
